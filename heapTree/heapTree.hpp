...
public:
    ...
    bool extract_root_element();

private:
    bool compare(T, T);
    void max_min_heapify(std::size_t);
...
template <typename T, bool Compare>
void HeapTree<T, Compare>::max_min_heapify(std::size_t index) {
    std::size_t max_min_index = index;
    std::size_t left_index = this->left(index);
    std::size_t right_index = this->right(index);
    std::size_t last_index = this->arr.size() - 1;
    if(left_index <= last_index && this->compare(this->arr[max_min_index], this->arr[left_index])) {
        max_min_index = left_index;
    }
    if(right_index <= last_index && this->compare(this->arr[max_min_index], this->arr[right_index])) {
        max_min_index = right_index;
    }
    if(max_min_index != index) {
        std::swap(this->arr[max_min_index], this->arr[index]);
        max_min_heapify(max_min_index);
    }
};

template <typename T, bool Compare>
bool HeapTree<T, Compare>::extract_root_element() {
    if(this->height() > -1) {
...
template <typename T, bool Compare>
int HeapTree<T, Compare>::linear_search(T elem) const {
